...
let colorIndex = 0;
// 線條種類數組
const linkTypes = ['#FFC14F', '#FFA84F', '#FFD74F', '#FF894F', '#FFEC4F'];
let linkTypeIndex = 0;
...
function init() {
        ...
        new go.Node('Position', {
            ...
            contextMenu: $(
                ...
                $(
                    'ContextMenuButton',
                    $(go.TextBlock, '新增小節點'),
                    { click: (e, obj) => addNodeAtBackground(e, 16) } // 新增小節點，大小為16
                ),
                $(
                    'ContextMenuButton',
                    $(go.TextBlock, '新增中等節點'),
                    { click: (e, obj) => addNodeAtBackground(e, 32) } // 新增中等節點，大小為32
                ),
                $(
                    'ContextMenuButton',
                    $(go.TextBlock, '新增大節點'),
                    { click: (e, obj) => addNodeAtBackground(e, 48) } // 新增大節點，大小為48
                )
            ),
        }).add(
            new go.Picture({
    ...
    function addNodeAtBackground(e, size) {
        ...
        const newNodeData = {
            category: 'PICTURE',
            loc: go.Point.stringify(clickPoint),
            image: 'yourImage.png', // 替換為實際圖片URL
            desiredSize: go.Size.stringify(new go.Size(size, size)), // 設定desiredSize
        };
        // 添加新節點到模型
